...
> 用途：把产品说明和架构说明拆成小功能、小版本和真实验收
> 状态原则：只有已经得到真实证据的功能才能标记完成
> 配套文档：01 产品说明、02 架构说明、04 测试用例与自测记录、05 实际实现说明。
...
- 产品说明只描述产品行为、页面、业务规则和验收。
- 产品说明不混入数据库、接口和代码实现。

架构约束：

- 架构说明描述唯一权威、契约、状态、数据和技术红线。
- 架构说明不替产品负责人决定页面和业务范围。
...
- 已生成答案可以正常展示。
- 每次模型调用能够获得缓存未命中输入、缓存命中输入和输出 Token。
- 不伪造 Usage。

架构约束：

- 只读检查 Docker、WSL2、CPU、内存、磁盘、端口和网络。
- 流式调用使用 `include_usage`。
- 请求显式设置 `thinking.type=enabled` 和 `reasoning_effort=high`。
- 保存 Provider 原始 Usage 样例。
- 不安装正式应用。
- 不修改既有容器。
...
- Run 启动固定 release_id。
- conversation 不永久固定 Release。
- Trace 只追加并结束于 done 或 error。
- 不接入 Agent 和模型。
...
- 用户看到真实流式回答。
- 本轮运行卡显示三类 Token、延迟和 Estimated Cost。
- Usage 异常时答案继续显示并明确标错。

架构约束：

- 统一 DeepSeek Adapter。
- 每次调用保存 CloudCallSnap。
- 缺失 Usage 保持 null并创建 Candidate。
- 不用字符估算冒充 Provider Usage。
...
- 时间、输入、回答和 Run 证据可见。
- 价格和成本统一以人民币展示。
- 不借此提前实现 Skill、RAG、MCP、工单、人机共驾或成本策略页面。

架构约束：

- 气泡抽屉与平台管理复用 `GET /api/runs/{run_id}`。
- 新 Run 追加 `user_message_received` 与 `assistant_response_completed`。
- 历史 Run 只从消息事实回显输入／输出，不补写历史 Trace。
- 官方 USD 原价、7.20 固定演示汇率和人民币单价进入新 Snap 快照。
- 旧 USD Snap 通过只读兼容字段显示人民币，不修改原始记录。
...
- RagVersion 固定原文、Chunk、模型和索引。
- Gate 0 确认 Embedding 资源。
- 不支持文件和网页数据源。
...
- Streamable HTTP。
- Schema hash 随 Release 固定。
- 调用生成外部 Snap。
- YIAI Center 不运行 Git MCP 和 stdio；独立部署层固定上游 commit 并提供远程 HTTP。
- 不接动态写 Tool。
...
- 单一输出权状态机。
- AI_RESUMING 创建新 Run并使用当前 Active Release。
- HUMAN_ACTIVE 时 AI 不回复。
...
- 人工确认根因和关闭。
- Evaluation 比较质量、Token、延迟和成本。
- 不自动改配置和发布。

架构约束：

- Darwin 和 Evaluation 的每次云调用也生成 CloudCallSnap。
- Darwin 作为显式高级专家工作流，可以使用 `deepseek-v4-pro` 思考模式。
- EvalRun 与真实 Run 分开标识。
- 回放使用固定输入和上下文。
...
- Run 汇总只来自 CloudCallSnap。
- CostPolicyVersion 随 Release 固定。
- Guardrail 服务端执行并进入 Trace。
- 缺失 Usage 时成本为 null。
