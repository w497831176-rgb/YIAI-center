...
> 当前部署：已部署并通过后端自测
> 配套文档：01 产品说明、02 架构说明、03 版本规划、04 测试用例与自测记录。
...
- 三个 TAB，无登录。
- 每条消息创建 Run。
- Run 固定当时 Active Release。
- 旧会话新消息使用最新 Active Release。
- 历史气泡保持旧 Release。
- 人工发布和回滚。
- 唯一 Router。
- 一个 Run 只选择一个 Agent。
- V4-Flash 思考模式。
- 每次云调用独立 Snap。
- 三类 Token 和 Estimated Cost。
- 历史对话列表和消息时间戳。
- AI 气泡右侧 Run 详情抽屉。
- Trace 保存新 Run 的完整输入和客服回答。
- 每个云调用 Trace 步骤内显示人民币 Snap 成本，底部汇总 Run。
- Usage 缺失时不丢答案、不编造数字。
- 没有自动升级 V4-Pro。
- 没有保存隐藏思考内容。
- 没有第二套 Release、Runtime 和成本权威。
- 没有生产级权限、安全和运维平台。

## 12. 当前明确未实现

以下功能仍是产品说明中的后续规划，不是 V0.5.5 已完成功能：
