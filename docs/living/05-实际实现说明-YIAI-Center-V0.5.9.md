...
- 完整源码映射环境内 30 项 unittest 全部通过，耗时 4.103 秒。
- 新地址健康与 Agent API 从开发机访问成功。
- 非白名单 MCP Tool 的 Agent PUT 请求返回 400。
- Agent 草稿原样保存后 Active Release 未变化。
- 容器重启后 Agent 绑定保持。
- Headless Edge 等待 5 秒后成功执行页面模块并渲染首屏，DOM 包含 `app-shell` 和 `chat-form`。
...
平台管理已统一采用“卡片总览 + 顶部主操作 + 卡片管理动作 + 独立弹层”的信息架构：

- Agent：右上角新增；卡片显示 Skill、RAG、MCP Tool 和预置 Tool 数量；卡片提供配置和删除草稿。
- Release：右上角创建候选版本；卡片提供查看变更、人工发布或回滚。
- Skill：右上角提供 GitHub 导入和新增；卡片提供编辑、校验和停用。
- RAG：右上角新增知识文档；卡片提供编辑、检索测试、校验和停用。
- MCP：右上角新增 Server；卡片提供编辑、连接测试、Tool 测试和停用。
- Run 与 Trace：每次运行是一张卡片，从卡片打开完整 Trace 详情。

垂直 Agent 同时新增真实创建和删除草稿 API。新 Agent 由服务端生成稳定 ID，可装配四类能力并进入下一个候选 Release。Router 不再只认三个固定 ID，而是从当前 Release 的 Agent 名称和业务说明中动态选择。

部署环境闭环自测临时创建 `agent_65e4d80942894573b39852e2d7fdbc4e`，Agent 数量从 3 变为 4；随后删除草稿恢复为 3。删除响应返回 `active_release_unchanged=true`，整个过程没有改写 Active Release 或历史 Run。
